package com.mypkg.todolist.userinterface.adapter
...
import com.mypkg.todolist.databinding.ItemTaskBinding
...
class TasksAdapter:
    ...
    private val callback = object : DiffUtil.ItemCallback<TaskCategoryInfo>(){
        override fun areItemsTheSame(
            ...
            return oldItem.myTaskInfo.id == newItem.myTaskInfo.id
        }
    ...
    inner class MyViewHolder(private val itemTaskBinding: ItemTaskBinding) : RecyclerView.ViewHolder(itemTaskBinding.root){
        fun bind(taskCategoryInfo: TaskCategoryInfo){
            ...
            itemTaskBinding.isCompleted.setOnCheckedChangeListener{_,it->
                taskCategoryInfo.myTaskInfo.status = it
                onTaskStatusChangedListener?.let {
                    it(taskCategoryInfo.myTaskInfo)
                }
            }
    ...
    private var onTaskStatusChangedListener :((MyTaskInfo)->Unit)?=null
    fun setOnTaskStatusChangedListener(listener : (MyTaskInfo)->Unit){
